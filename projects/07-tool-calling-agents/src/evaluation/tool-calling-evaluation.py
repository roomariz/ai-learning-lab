...
# Load a small slice so the script starts quickly.
...
def search_symptoms(symptom: str, duration: str) -> str:
    """Search possible conditions for a symptom and duration."""
    return json.dumps(
        {
            "conditions": [
                {
...
def suggest_specialist(condition: str, urgency: str) -> str:
    """Suggest appropriate specialist care."""
    return json.dumps(
        {
            "condition": condition,
...
def analyze_medical_report(report_text: str) -> str:
    """Analyse a medical report and suggest follow-up."""
    return json.dumps(
        {
            "risk_level": "moderate",
            "recommendations": [
...
for i, r in enumerate(results):
    ...
    expected_tool = (
        "NO TOOL" if r["expected_tool"] is None else r["expected_tool"]
    )
    actual_tool = (
        "NO TOOL" if r["actual_tool"] is None else r["actual_tool"]
    )
    score = metric.score(
        llm=judge_llm,
        user_query=r["query"],
