...
def render_extract_keywords(result):
    ...
    for kw in data["keywords"]:
        console.print(f"  [green]OK[/green] {kw}")
...
def render_chuck_norris(result):
    ...
    console.print(f"[green]OK[/green] ID: {data['id']}")
    console.print(f"[green]OK[/green] [link]{data['url']}[/link]")


RENDERERS = {
    "search_docs": render_search,
    "read_document": render_read_document,
    "summarise_document": render_summarise_document,
    "extract_keywords": render_extract_keywords,
    "answer_question": render_answer_question,
    "get_chuck_norris_fact": render_chuck_norris,
}
...
tools = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Summarise the content of a document",
            "parameters": {
                "type": "object",
                "properties": {"doc_id": {"type": "string"}},
                "required": ["doc_id"]
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "get_chuck_norris_fact",
            "description": "Get a random Chuck Norris fact from an external API",
            "parameters": {
                "type": "object",
                "properties": {},
...
def show_debug_trace(response):
    console.print(Panel("[yellow]DEBUG: Raw Response[/yellow]", style="yellow"))
    import json as j
    console.print(j.loads(response.model_dump_json()))
    ...
    for i in range(max_iterations):
        ...
        if message.get("tool_calls"):
            for tool_call in message["tool_calls"]:
                ...
                console.print(f"\n[yellow]... Running {function_name}...[/yellow]")

                try:
                    log_tool_call(function_name, arguments)
                    result = execute_tool(function_name, arguments)
                    log_tool_result(function_name, result)
                    console.print(f"[green]OK {function_name} completed[/green]")
